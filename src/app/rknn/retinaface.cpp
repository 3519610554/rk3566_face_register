...
static int post_process_retinaface(rknn_app_context_t *app_ctx, image_buffer_t *src_img, rknn_output outputs[], retinaface_result *result, letterbox_t *letter_box) {
    ...
    {
        spdlog::error("model_shape error!!!");
        return -1;
    ...
    for (int i = 0; i < validCount; ++i) {
        if (last_count >= 128) {
            spdlog::warn("Warning: detected more than 128 faces, can not handle that");
            break;
        }
...
int inference_retinaface_model(rknn_app_context_t *app_ctx, image_buffer_t *src_img, retinaface_result *out_result) {
    ...
    if (img.virt_addr == NULL) {
        spdlog::error("malloc buffer size:{} fail!", img.size);
        return -1;
    }

    ret = convert_image_with_letterbox(src_img, &img, &letter_box, bg_color);
    if (ret < 0) {
        spdlog::error("convert_image fail! ret={}", ret);
        return -1;
    }
    ...
    if (ret < 0) {
        spdlog::error("rknn_input_set fail! ret={}, inputs size: {}", ret, inputs[0].size);
        return -1;
    }
    ...
    if (ret < 0) {
        spdlog::error("post_process_retinaface fail! ret={}", ret);
        return -1;
    }
